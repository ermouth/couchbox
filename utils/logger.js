...
const sendMail = (recipients = config.get('couchbox.mail.recipients'), mmailMessagesg, subj, from = config.get('couchbox.mail.from')) => {
  ...
  return execBash('printf "${mailHeaders}\n${mailMessage}" | sendmail "$recipients"', {
    mailHeaders,
    mailMessage,
    recipients
  });
